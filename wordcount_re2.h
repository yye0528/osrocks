...
{
	string match;
	re2::RE2 alphaPat("([a-z])");
	re2::RE2 wordPat("\\b([\\w'-]*)\\b");
	re2::RE2 sentencePat("([^ ]*)\n");
	...
	re2::StringPiece str(inputStr);

	while(re2::RE2::FindAndConsume(&str, wordPat, &match)){
		...
		for (unsigned int i=0; i<word.length(); i++){
			...
			if(re2::RE2::FullMatch(c,alphaPat))
	...
	re2::StringPiece input(inputStr);
	while(re2::RE2::FindAndConsume(&input,sentencePat,&match)){
		sentenceCount++;
	}
